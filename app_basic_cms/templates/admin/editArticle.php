...
<form action="admin.php?action=<?php echo $results['formAction']?>" method="post">
  ...
  <ul>
    <li>
      ...
      <input type="text" name="title" id="title" placeholder="Name of the article" required autofocus max="255" value="<?php echo htmlspecialchars($results['article']->title) ?>" />
    </li>
    ...
    <li>
      ...
      <input type="date" name="publicationDate" id="publicationDate" placeholder="YYYY-MM-DD" required maxlength="10"
        value="<?php $results['article']->publicationDate ? date('Y-m-d', $results['article']->publicationDate) : "" ?>"/>
    </li>
  </ul>
...
<?php if ($results['article']->id) { ?>
  <p class="bottom">
    <a href="admin.php?action=deleteArticle&amp;articleId=<?php echo $results['article']->id ?>"
      onclick="return confirm('Delete This Artcle?')">
        Delete This Article?
    </a>
  </p>
<?php } ?>
